...
(define (new-eval e)
  (eval (list '(lambda-rec (fun x)
              (if (pair? x)
                  (let [sub1 (fun (car (cdr x)))]
                    (let [sub2 (fun (car (cdr (cdr x))))]
                      (cond [(eq? (car x) '+) ((lambda (x y) (+ x y)) sub1 sub2)]
                            [(eq? (car x) '-) ((lambda (x y) (- x y)) sub1 sub2)]
                            [(eq? (car x) '*) ((lambda (x y) (* x y)) sub1 sub2)]
                            [(eq? (car x) '/) ((lambda (x y) (/ x y)) sub1 sub2)])))
                      x)) e)))
...
(define (test)
  (define (compare res e)
    (if (= (new-eval e) res)
        (printf "OK ~%")
        (printf "Wrong answer ~%")))
    
  (compare 2 '2)
  (compare 5   '(cons '+ (cons 2 (cons 3 null))))
  (compare 8   '(cons '- (cons 10 (cons 2 null))))
  (compare 25   '(cons '* (cons 5 (cons 5 null))))
  (compare 3   '(cons '/ (cons 9 (cons 0 null))))
  (compare 8   '(cons '+ (cons (cons '+ (cons 2 (cons 3 null))) (cons 3 null))))
  (compare 94  '(cons '+ (cons (cons '- (cons 5 (cons 1 null)))
                               (cons (cons '* (cons 9 (cons 10 null)))
                                null))))
  (compare 1/2  '(cons '/ (cons (cons '- (cons 5 (cons 1 null)))
                               (cons (cons '* (cons 4 (cons 2 null)))
                                     null))))
  (compare 121  '(cons '+ (cons (cons '- (cons 23 (cons (cons '* (cons 2 (cons 1 null))) null)))
                                (cons (cons '/ (cons 500 (cons (cons '/ (cons 10 (cons 2 null))) null)))
                                      null))))
)
