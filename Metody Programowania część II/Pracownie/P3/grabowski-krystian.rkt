...
(define (solve-ax+by=1 a b)
  (if (= a 0)
      (cons 0 1)
      (let ([z (solve-ax+by=1 (modulo b a) a)])
        (let ([x (car z)]
              [y (cdr z)])
          (cons (- y (* (floor (/ b a)) x)) x)))))
...
(define (make-signed-message mess sig)
  (cons mess sig))

(define (signature msm)
  (cdr msm))
...
(define (encrypt-and-sign message recipient-public-key sender-private-key)
  (let ([x (RSA-encrypt message recipient-public-key)])
    (let ([y (compress x)])
      (let ([z (RSA-convert-list (cons y null) sender-private-key)])
        (make-signed-message x (car z))))))

(define (authenticate-and-decrypt cyphertext recipient-private-key sender-public-key)
  (let ([x (compress (message cyphertext))]
        [y (RSA-unconvert-list (cons (signature cyphertext) null) sender-public-key)])
    (if (= x (car y))
