...
eval input e = case e of
          ...
          EUnary p unaryop e1 -> if (zerochck input e) then Value (num_c input e) else RuntimeError
          EBinary p binaryop e1 e2 -> case binaryop of
                               BAdd -> if (zerochck input e) then Value (num_c input e) else RuntimeError
                               BSub -> if (zerochck input e) then Value (num_c input e) else RuntimeError
                               BMul -> if (zerochck input e) then Value (num_c input e) else RuntimeError
                               BDiv -> if (zerochck input e) then Value (num_c input e) else RuntimeError
                               BMod -> if (zerochck input e) then Value (num_c input e) else RuntimeError
          ELet    p var e1 e2 -> if (zerochck input e) then Value (num_c input e) else RuntimeError
          EIf     p e1 e2 e3 -> if (zerochck input e) then Value (num_c input e) else RuntimeError 
          otherwise -> Value 10
...
zerochck input e = case e of
          ...
          EBinary p binaryop e1 e2 -> case binaryop of
                               BDiv -> if (num_c input e2 == 0) then False else ((zerochck input e1) && (zerochck input e2))
                               BMod -> if (num_c input e2 == 0) then False else ((zerochck input e1) && (zerochck input e2))
                               otherwise -> ((zerochck input e1) && (zerochck input e2))
          ELet   p var e1 e2 -> (zerochck input e1) && (zerochck input e2)
          EIf    p e1 e2 e3 -> if (zerochck input e1) && (bool_c input e1 == True) then (zerochck input e2) else (zerochck input e3)

bool_c input e = case e of
          EBool   p bool -> bool
          EVar    p var  -> if (tupl input var) == 1010101 then True else False
          EUnary p unaryop e1 -> not(bool_c input e1)
          EBinary p binaryop e1 e2 -> case binaryop of
                                          BEq -> (num_c input e1) == (num_c input e2)
                                          BNeq -> (num_c input e1) /= (num_c input e2)
                                          BLt -> (num_c input e1) < (num_c input e2)
                                          BGt -> (num_c input e1) > (num_c input e2)
                                          BLe -> (num_c input e1) <= (num_c input e2)
                                          BGe -> (num_c input e1) >= (num_c input e2)
                                          BAnd -> (bool_c input e1) && (bool_c input e2)
                                          BOr -> (bool_c input e1) || (bool_c input e2)
          ELet    p var e1 e2 -> if (evtype e1 == True) then bool_c (input ++ [(var,num_c input e1)]) e2 else
                                     if (bool_c input e1 == True) then bool_c (input ++ [(var,111)]) e2 else bool_c (input ++ [(var,100)]) e2
          EIf   p e1 e2 e3 -> if (bool_c input e1) then (bool_c input e2) else (bool_c input e3)
          otherwise -> True


num_c input e = case e of
          ENum    p integer -> integer
          EVar    p var  -> tupl input var
          EUnary p unaryop e1 -> -(num_c input e1)
          EBinary p binaryop e1 e2 -> case binaryop of
                                          BAdd -> (num_c input e1) + (num_c input e2)
                                          BSub -> (num_c input e1) - (num_c input e2)
                                          BMul -> (num_c input e1) * (num_c input e2)
                                          BDiv -> (num_c input e1) `div` (num_c input e2)
                                          BMod -> (num_c input e1) `mod` (num_c input e2)
          ELet    p var e1 e2 -> if (evtype e1 == True) then num_c (input ++ [(var,num_c input e1)]) e2 else
                                     if (bool_c input e1 == True) then num_c (input ++ [(var,111)]) e2 else num_c (input ++ [(var,100)]) e2
          EIf   p e1 e2 e3 -> if (bool_c input e1) then (num_c input e2) else (num_c input e3)
          otherwise -> 1
...
evtype e = case e of
          ...
          EUnary p unaryop e -> if (show unaryop == "not") then False else True
          EBinary p binaryop e1 e2 -> if (binbool binaryop == True) then True else False
          EIf     p e1 e2 e3 -> if (evtype e2 == True) then True else False
          otherwise -> True
