...
class Custadd(Gtk.Window):
    def __init__(self, gyms):
        ...
        self.grid.attach_next_to(self.entry21, label21,
                                 Gtk.PositionType.RIGHT, 50, 10)
        ...
        self.grid.attach_next_to(self.combobox, label3,
                                 Gtk.PositionType.RIGHT, 50, 8)
        self.add = Gtk.Button(label="Add customer")
        self.add.connect("clicked", self.add_m)
        self.grid.attach_next_to(self.add, self.combobox,
    ...
    def add_m(self, widget):
        ...
        f_name = self.entry1.get_text()
        s_name = self.entry2.get_text()
        cust_id = self.entry21.get_text()
        sel_gym = self.combobox.get_active_text()

        if sel_gym is None or f_name == "" or s_name == "" or cust_id == "":
            ...
            for gym in self.gyms_add:
                self.cur.execute('SELECT * FROM customer WHERE name = ?',
                                 (gym,))
            ...
            if self.is_ok:
                self.cur.execute('''INSERT INTO customer(fname, sname, id, name)
                                 VALUES(?, ?, ?, ?)''', (f_name,
                                 s_name, cust_id, sel_gym))
