...
(define (append xs1 xs2)
  (printf "~v ~%" xs1)
  (eval (list '(lambda-rec (append x y) (if (null? x)
                                            y
                                            (cons (car x) (append (cdr x) y))))
         xs1
         xs2)))

;(eval '((lambda-rec (map f xs) (if (null? xs)
;                                      null
;                                      (cons (f (car xs)) (map f (cdr xs)))))
;          (lambda (x) 10) (cons 4 (cons 5 null))))





;(eval '((lambda-rec (reverse xs)
;                      (if (null? xs)
;                          xs
;                          ((lambda-rec (append x y) (if (null? x)
;                                            y
;                                            (cons (car x) (append (cdr x) y))))
;                           (reverse (cdr xs))(cons (car xs) null))))
