;;https://github.com/evoL/notatki/blob/master/Programowanie%20(L)/06.%20Sk%C5%82adnia%20je%CC%A8zyko%CC%81w%20programowania.md

;;Homoikoniczność (łac. homo - ten sam, icon - przedstawienie) - to cecha języków programowania,
;;w których reprezentacja programu jest jednocześnie podstawową strukturą danych wykorzystywaną w języku.
...
(define (arith-rpn lista)
  (if (number? lista) lista
      (flatten (list (arith-rpn (cadr lista))
                     (arith-rpn (caddr lista))
                     (car lista)))))

(define (arith-rpn2 lista)
  (define (onp xs acc)
    (cond
      [(number? xs) (cons xs acc)]
...
(define (eval-rpn exp)
  (define (pom l s)
    (if (null? l)
        (car s)
        (cond [(const? (car l)) (pom (cdr l) (push (car l) s))]
              [else (pom (cdr l) (push ((op->proc (car l)) (pop s) (pop (rest s))) s))])))
  (pom exp null))
